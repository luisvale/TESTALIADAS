...
class SaleSubscription(models.Model):
    ...
    contract_name = fields.Char(string='Contato N°', tracking=True, help='Número de contrato', copy=False, store=True, readonly=True)
    ...
    def start_subscription(self):
        ...
        #self._send_mail_to_team()
        return res

    def _send_mail_to_team(self):
        for record in self:
            if record.team_id:
                member_ids = record.team_id
    ...
    def _filtered_local(self, line):
        if line.order_line_id:
            ...
            line_local = order_line_id._filtered_local()
            return line_local
        else:
            return False
    ...
    def _prepare_invoice_line_rental(self, list_lines, lines_locals, fiscal_position, date_start=False, date_stop=False):
        ...
        if lines_locals:
            for local in lines_locals:
                if local.order_line_id:

                    lines, pending_invoiced = local._find_move_line(line.analytic_account_id, line.analytic_account_id.recurring_next_date)
                    date_init = (local.order_line_id.pickup_date - timedelta(hours=6)).date() if not lines else lines.mapped('subscription_period_end')[0] + timedelta(days=1)
                    #
                    # if not local.date_init:
                    #     #pickup_date = local.order_line_id.pickup_date + timedelta(hours=6)
                    #     pickup_date = local.order_line_id.pickup_date - timedelta(hours=6)
                    # else:
                    #     pickup_date = local.date_init
                    ...
                    amount_invoice += (price_subtotal / 30 * days)
    ...
    def _prepare_invoice_line_aliadas(self, values, line, manual=False):
        order_id = line[0].order_line_id.order_id if line[0].order_line_id else False
        if order_id and order_id.is_rental_order:
            if line.rental_type == 'm2':
                locals_ids = self.recurring_invoice_line_ids.mapped('order_line_id')._filtered_local()
                recurring_next_date = self.recurring_next_date
                amount_invoice = 0.0
                tags = []
                if locals_ids:
                    for local in locals_ids:
                        pickup_date = local.pickup_date + timedelta(hours=6)
                        days = (recurring_next_date - pickup_date.date()).days
                        price_subtotal = local.price_subtotal if line.currency_id == line.currency_external_id else local.amount_convert
                        # Precio a facturar
                        # Cuándo se debe dividir entre 30?
                        amount_invoice += (price_subtotal / 30 * days)
            ...
            elif line.rental_type in ('consumption', 'consumption_min', 'consumption_fixed'):
                values['price_unit'] = line.price_subtotal if line.currency_id == line.currency_external_id else line.amount_convert

            elif line.rental_type in ('rental_min', 'rental_percentage', 'rental_percentage_top'):
                values['price_unit'] = line.price_subtotal if line.currency_id == line.currency_external_id else line.amount_convert
            ...
            if lines and manual:
                ...
                subscription_next_invoiced = line.analytic_account_id._next_date()
                line.sudo().write({'date_init': subscription_period_start, 'date_end': subscription_period_end})
    ...
    def _prepare_invoice_aliadas(self, lines, document_type_sale_id, currency_id):
        ...
        if lines:
            if lines[0].order_line_id:
                if lines[0].order_line_id.order_id:
                    if lines[0].order_line_id.order_id.commercial_id:
                        invoice['commercial_id'] = lines[0].order_line_id.order_id.commercial_id.id
        return invoice
