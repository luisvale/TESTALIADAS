...
class AccountMove(models.Model):
    ...
    cai_supplier_id = fields.Many2one('res.cai.trigger', string='CAI Proveedor', store=True, tracking=True, compute='_compute_cai_id')
    ...
    cai_supplier = fields.Char(string='Campo invalido', help='CAI de proveedor a buscar')
    ...
    def _compute_document_type_id(self):
        for record in self:
            ...
            if record.document_type_sale_id:
                ...
                if record.document_type_sale_id.code == '00':
                    record.check_cai = False
    ...
    def _compute_check_cai(self):
        for record in self:
            ...
            if record.move_type in ('out_invoice', 'out_refund'):
                check_cai = True
            record.check_cai = False if record.document_type_code == '00' else check_cai
    ...
    def _create_sequence(self):
        for inv in self:
            if inv.move_type in ('out_invoice', 'out_refund') and inv.check_cai and inv.document_type_code != '00':
            ...
            elif inv.move_type in ('in_invoice', 'in_refund') and inv.check_cai and inv.document_type_code != '00':
                if not inv.document_type_id:
                    raise ValidationError(_("Si tiene el check para validar C.A.I. debe seleccionar el tipo de documento."))
    ...
    def _compute_cai_id(self):
        for record in self:
            ...
            if cai_id and record.move_type in ('in_invoice', 'in_refund'):
                cai_supplier_id = self.env['res.cai.trigger'].sudo()._create_mirror(record, cai_id)
            record.cai_id = cai_id
            record.cai_supplier_id = cai_supplier_id
