...
def _create_maintenance_periodicity(master, level_to_assign, self):
    ...
    data_to_create = []
    if level_to_assign:
        first = level_to_assign[0]
        line = first
        level_ids = line['lines'].mapped('level_id')
        max_days = max(line['lines'].mapped('days'))
            ...
            for data_list in data_to_create:
                try:
                    self.sudo().write({
                        ...
                        'maintenance_type': master.maintenance_type,
                        'maintenance_team_id': master.maintenance_team_id.id,
                        'color': master.color,
                        'user_id': master.user_id.id,
                        'narration': master.narration,
                        'location_init_id': master.location_init_id.id,
                        'location_end_id': master.location_end_id.id,
                        'company_id': master.company_id.id,
                        'currency_id': master.currency_id.id,
                    })

                    for pl in master.periodicity_line_ids:
                        pl.sudo().copy(default={'maintenance_id': self.id})

                    for el in master.equipment_line_ids:
                        el.sudo().copy(default={'maintenance_id': self.id})

                    for ml in master.material_line_ids:
                        ml.sudo().copy(default={'maintenance_id': self.id})

                    if self:
                        _logger.info("MANTENIMIENTO ALIADAS: Mant.Periódico creado con ID %s" % self.id)
        ...
        return "Lamentablemente no hubo niveles a asignar"
