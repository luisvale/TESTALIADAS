...
class Invite(models.TransientModel):
    ...
    #                                domain=[('type', '!=', 'private'),('state','in',['draft','done'])])

    partner_prospect_id = fields.Many2one('res.partner', string='Prospectos de clientes', domain=[('active','=',False), ('state','=','draft')])
    #
    #
    # def add_partner_id(self):
    #     self.sudo().write({'partner_ids': [(6, 0, self.partner_prospect_id.ids)]})
    #     #self.partner_ids += self.partner_prospect_id
    #
    #     return {
    #         u'name': u'Invitar seguidor',
    #         u'type': u'ir.actions.act_window',
    #         u'view_mode': u'form',
    #         u'target': u'new',
    #         u'res_model': u'mail.wizard.invite',
    #         u'res_id': self.id
    #     }
    #
    ...
    def add_followers(self):
        ...
        for wizard in self:
            ...
            if wizard.partner_prospect_id:
                partner_ids += wizard.partner_prospect_id
            new_partners = partner_ids - document.sudo().message_partner_ids
