                ...
                for level_id in lack_levels:
                    for g in group_factory:
                        ...
                        if level_id.id not in leve_ids.ids:
                            ...
                            nex_date_start = child.date_start + timedelta(days=days-interval)
...
def _create_maintenance_periodicity(master, child, upper_level, level_to_assign):
    ...
    for level_list in level_to_assign:
        ...
        if not child:
            ...
            _logger.info("No llega nivel TOP")
            new_date_start = child.date_start + timedelta(days=max_days)
            new_date_end = new_date_start + timedelta(days=interval)
        elif upper_level:
            _logger.info("Llego nivel TOP. Reinicio de proceso.")
            new_date_start = child.date_start + timedelta(days=max_days)
            new_date_end = new_date_start + timedelta(days=interval)
